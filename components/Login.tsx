...
const Login: React.FC = () => {
  const { users, dbStatus, dbError } = getStore();
        ...
        <form onSubmit={handleLogin} className="bg-white p-8 rounded-[3rem] shadow-2xl space-y-8 border border-white/10">
          ...
          <div className="space-y-4">
            <div className="flex justify-center gap-3">
              {[...Array(selectedUserId ? 4 : 0)].map((_, i) => (
                <div
                  key={i}
                  className={`w-4 h-4 rounded-full border-2 transition-all ${i < pin.length ? 'bg-emerald-500 border-emerald-500 scale-125' : 'bg-transparent border-slate-200'
                    }`}
                ></div>
              ))}
            </div>
          ...
          <div className="pt-4 border-t border-slate-100 mt-4">
            <div className="flex items-center gap-2 mb-1">
              <div className={`w-1.5 h-1.5 rounded-full ${dbStatus === 'connected' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Status: {dbStatus}</span>
            </div>
            {dbStatus === 'error' && <p className="text-[8px] text-rose-500 font-bold lowercase">{dbError}</p>}
          </div>
        </form>
      </div>
    </div>
