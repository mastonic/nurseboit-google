          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div className="md:col-span-1">
              <div className="aspect-[3/4] bg-slate-100 rounded-2xl overflow-hidden border border-slate-200">
                {scannedImage && <img src={scannedImage} className="w-full h-full object-cover" alt="Prescription" />}
              </div>
            </div>
          ...
          return (
            <div key={script.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-lg transition-all relative group overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-1">
                <h3 className="font-black text-xl text-slate-800 truncate pr-4">{patient?.lastName || 'Patient inconnu'}</h3>
                {script.imageUrl && (
                  <div className="w-10 h-10 bg-slate-50 rounded-lg flex items-center justify-center border border-slate-100 shadow-sm overflow-hidden">
                    <img src={script.imageUrl} className="w-full h-full object-cover opacity-50 hover:opacity-100 transition-opacity cursor-zoom-in" alt="Micro preview" />
                  </div>
                )}
              </div>
